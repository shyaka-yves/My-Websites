...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($first_name) || empty($last_name) || empty($email) || empty($password)) {
        ...
        if ($stmt->fetch()) {
            ...
            // Create user account
            if (registerUser($conn, $email, $password, $first_name, $last_name, $phone, $address)) {
...
<body>
    ...
    <script>
        ...
        function updatePasswordStrengthIndicator(strength) {
            ...
            indicator.className = `progress-bar bg-${colors[strength - 1] || 'danger'}`;
            indicator.style.width = `${(strength / 5) * 100}%`;
            indicator.textContent = texts[strength - 1] || 'Very Weak';
        }
